...
#ifdef GRB
    int grbError = GRBdelconstrs( lp->lp, idx.size(), &idx[0] );
    lp_check_for_grb_error( LPgrbDefaultEnv, grbError, __FILE__, __LINE__ );
    grbError = GRBupdatemodel( lp->lp);
    lp_check_for_grb_error( LPgrbDefaultEnv, grbError, __FILE__, __LINE__ );
...
void lp_add_cols(LinearProgram *lp, const int count, double *obj, double *lb, double *ub, char *integer, char **name)
{
    ...
    for ( int i=0 ; (i<count) ; ++i )
    {
        for ( int j=i+1 ; (j<count) ; ++j )
        {
            ...
            {
                fprintf( stderr, "Adding columns with repeated name %s indexes %d and %d", name[i], i, j );
                fflush( stdout );
                fflush( stderr );
                warned = true;
                break;
...
// converts a vector of string to a vector of char
char** lp_string_vchar(const std::vector< std::string >& strv)
{
    if ( strv.size() == 0 )
        return nullptr;
